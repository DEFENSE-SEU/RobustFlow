...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Evaluate solutions based on relevance and accuracy
        evaluated_solutions = await self.custom(input="Evaluate these solutions: " + ", ".join(solutions), instruction="Score each solution based on relevance and accuracy.")
        
        # Use self-consistency to select the best solution
        final_solution = await self.sc_ensemble(solutions=[sol for sol in evaluated_solutions['response'].split(',') if sol.strip()])
