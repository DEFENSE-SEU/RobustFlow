...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        review_response = await self.custom(input=f"Review these responses: {response['response']} and {answer_response['answer']}.", instruction="Evaluate the quality and relevance of the answers.")
        
        # Combine responses for ensemble selection
        ensemble_response = await self.sc_ensemble(solutions=[review_response['response'], answer_response['answer']])
