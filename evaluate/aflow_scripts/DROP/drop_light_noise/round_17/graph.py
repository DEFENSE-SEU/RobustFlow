...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        review_response = await self.review(responses=[response['response'], answer_response['answer']])
        
        # Combine reviewed responses for ensemble selection
        ensemble_response = await self.sc_ensemble(solutions=review_response['reviewed_solutions'])
        
        # Final output using the ensemble response
