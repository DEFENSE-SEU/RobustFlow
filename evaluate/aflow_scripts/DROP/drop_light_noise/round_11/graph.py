...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Self-ask mechanism to refine the question
        refined_question = f"Refine the question: {problem}"
        refined_response = await self.custom(input=refined_question, instruction="")
        
        # Generate a step-by-step answer using AnswerGenerate
        answer_response = await self.answer_generate(input=refined_response['response'])
