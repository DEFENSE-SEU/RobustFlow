...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Validate the extracted answer before finalizing
        validation = await self.custom(input=summary['response'], instruction="Validate the following answer for correctness.")
        
        # Use self-consistency to select the best response
        final_solution = await self.sc_ensemble(solutions=[validation['response']])
