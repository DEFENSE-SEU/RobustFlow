...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        """
        # Generate a step-by-step breakdown of the answer
        breakdown = await self.answer_generate(input=problem)
        # Use the breakdown to create multiple solutions
        solutions = await self.custom(input=problem + breakdown['thought'], instruction="")
        # Use ScEnsemble to select the best solution from multiple generated responses
        final_solution = await self.sc_ensemble(solutions=[solutions['response']])
