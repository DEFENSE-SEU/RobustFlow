...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Generate a summary of responses for better selection
        summary = await self.custom(input="Summarize the following responses: " + " | ".join(responses), instruction="")
        
        # Use self-consistency to select the best solution
        final_solution = await self.sc_ensemble(solutions=[summary['response']] + responses)
